...
export const createRajeshHouse = async (req, res) => {
    try {
        ...
        if (!clientId || !uniqueId || !username) {
            console.error("[createRajeshHouse] Missing required fields");
        ...
        if (existingForm) {
            return res.status(200).json({
                success: true,
                message: "Rajesh House form already exists (duplicate submission prevented)",
                data: existingForm,
                isDuplicate: true
            });
        }
        ...
        res.status(500).json({
            success: false,
            message: "Failed to create Rajesh House form",
            error: error.message
        });
    }
};
...
export const getRajeshHouseById = async (req, res) => {
    try {
        ...
        console.error("[getRajeshHouseById] Error:", error);
        res.status(500).json({
            success: false,
            message: "Failed to fetch Rajesh House form",
            error: error.message
        });
    }
};
...
export const getAllRajeshHouse = async (req, res) => {
    try {
        ...
        if (!clientId) {
            console.error("[getAllRajeshHouse] Missing clientId");
            return res.status(400).json({
                success: false,
        ...
        console.error("[getAllRajeshHouse] Error:", error);
        res.status(500).json({
            success: false,
            message: "Failed to fetch Rajesh House forms",
            error: error.message
        });
    }
};
...
export const updateRajeshHouse = async (req, res) => {
    try {
        ...
        // Handle spreadsheet data - mark when it's synced from form
        if (updateData.spreadsheetData) {
            updateData.spreadsheetData.lastUpdatedAt = new Date();
            updateData.spreadsheetData.syncedFromForm = true;
            ('[updateRajeshHouse] Received spreadsheetData with',
                updateData.spreadsheetData.rows?.length, 'rows');
        }
        ...
        ('[updateRajeshHouse] Updating form:', {
            formId: form._id,
            username,
            hasSpreadsheetData: !!updateData.spreadsheetData
        });
        ...
        if (!updatedForm) {
            console.error("[updateRajeshHouse] Failed to update form:", id);
        ...
        res.status(200).json({
            success: true,
            message: "Rajesh House form updated successfully",
            data: updatedForm
        });
    } catch (error) {
        console.error("[updateRajeshHouse] Error:", error);
        res.status(500).json({
            success: false,
            message: "Failed to update Rajesh House form",
            error: error.message
...
export const managerSubmitRajeshHouse = async (req, res) => {
    try {
        ...
        console.error("[managerSubmitRajeshHouse] Error:", error);
        res.status(500).json({
            success: false,
